// LiveAsk Platform Runtime â€” assets/liveask-widget.js migrated for Stage A.
//
...
// implementation â€” extraction, not a rewrite. What changed is WHERE this
...
// window.__liveAskPendingConfig immediately before appending this script â€”
...
(function(){
  ...
  // LiveAsk_Deployment_Handover.md) â€” data-liveask-site is read by the
  ...
  // hardcoded customer default â€” no tenant identity appears below,
  ...
  // Platform-controlled constant â€” never derived from DEPLOYMENT_COMPANY_NAME.
  ...
  // answering identity (.ask-identity, see beginIdentity() below) â€” no
  ...
  // No fallback of any kind, to any customer's Worker â€” CHANNEL Core must
  ...
  //     same shadow tree.
  //   host.$ / host.$$  â†’ always resolve against the real host document.
  ...
  //     website â€” that is not something Shadow DOM should, or does,
  //     prevent.
  var panel = {
  ...
  // ---- CSS delivery ----
  // Deliberately not an inline <style> element with textContent â€” that
  ...
  // the package and widget.css correctly declaring them. Root cause â€”
  ...
  // would try to load from the customer's own domain, not LiveAsk's â€”
  ...
  // The <link> fallback path below needs no equivalent fix â€” a
  ...
  // Stylesheet applied through shadowRoot.adoptedStyleSheets â€” the
  ...
  // for rendering â€” the declaration alone is not the same thing as the
  ...
  // and document.fonts.add() â€” deliberately not something that lives
  ...
  // which shadow tree the text lives in â€” Shadow DOM scopes style
  ...
  // rules are left in place, not removed â€” harmless redundancy for the
    ...
    faces.forEach(function (f) {
      try {
        ...
        fontFace.load().then(function (loaded) {
          ...
          console.error('LiveAsk: font failed to load â€”', f.url, err);
        });
      } catch (err) {
        console.error('LiveAsk: FontFace construction failed â€”', f.url, err);
      }
    });
  })();
  ...
  // markup â€” only WHERE this markup lives has changed, not what it is.
  // The known, accepted temporary exception this replaces â€” a hand-edited
  // static pre-JS placeholder/aria-label duplicated across three HTML
  // files â€” no longer applies at all: there is no static host-page markup
  ...
  // content (GEO, ChatGPT visibility, competitors in AI search) â€” the
  ...
  // back up â€” that's new copy nobody has asked for yet. The rotation
  ...
  // + send (moved down from Row 1) inside .ask-row2-right â€” so quickReply
  ...
  function detectInputInstruction(text){
    ...
    if (/\b(?:verification|validation|security|one[- ]time)\s+code\b[\s\S]{0,180}\b(?:enter|type|provide)\s+(?:it|that|the code)\b/i.test(value)) {
      return {
        ...
        isError: /\b(?:code|entry|value)\s+(?:is|was)\s+(?:invalid|incorrect|wrong|not valid)\b|\b(?:invalid|incorrect|wrong)\s+(?:code|entry|value)\b|\bdidn['â€™]?t match\b/i.test(value)
      };
    }
    ...
    // over the real next action â€” for example, "send a verification code to
    ...
    const fieldErrorPattern = new RegExp(
      '(?:' + selected.rule.term.source + ')[\\s\\S]{0,48}\\b(?:invalid|incorrect|wrong|didn[â€™\\\']?t match|not valid)\\b' +
  ...
  // on narrow viewports â€” both tenant-specific hangovers that
  ...
  // no viewport branching) â€” consistent with everything else rather
  ...
  // typing or submit â€” simply clicking into the empty field left it fully
  ...
  // still see the rotating prompt suggestions) â€” only a genuine user tap
  ...
  // "Reopen chat" tab) ----
  // Expands the panel to show what's already in conversationHistory â€” but
  ...
  // content â€” expanding with zero history briefly left a short box
  // with that button landing on top of the send button. The button
  // itself is gone (31 August 2026, replaced by the persistent
  // footer's "Hide Chat âŒƒ" control), but the underlying box-growth
  ...
  // visitor anyway â€” the rotating placeholder already is their
  // experience.
  ...
  input.addEventListener('focus', function(){
    if (autoFocusPending) {
      ...
      // focus too â€” killing rotation before it ever got to cycle even
      ...
      // here, so a genuine tap into an EMPTY box â€” with no rotation even
      // running (e.g. the "final"/post-conversation placeholder, or the
      // brief window before the first reply lands) â€” still added the
      ...
      // fading and the padding collapsing â€” the exact glitch reported live:
      ...
      // genuinely active (the only case this was ever meant to cover) â€”
      ...
      clearTimeout(rotateFadeTimeout);
      // Covers a genuinely NEW focus â€” e.g. tabbing into the field with a
  ...
  // focused â€” and clicking an already-focused element fires no new 'focus'
  ...
  // stop below (input event vs. focus event) â€” a genuine 'click' fires
  ...
  // Auto-focus on landing so the real cursor blinks immediately â€” desktop
  ...
  // instant the page finishes loading â€” often not even visibly firing
  ...
  // touch-primary device â€” skip the auto-focus there entirely. Desktop
  // (pointer: fine, no on-screen keyboard to disturb) keeps the original
  // courteous cursor-ready-on-landing touch, unchanged.
  //
  // IMPORTANT â€” this ONLY ever gates this one script-triggered focus() call
  // on page load. A visitor's own genuine tap into the input still opens
  // the keyboard exactly as any text field always does on any device â€”
  ...
  window.addEventListener('load', function(){
    ...
    input.blur();
    requestAnimationFrame(function(){
      input.focus({ preventScroll: true }); // real paint gap before refocus â€” back-to-back blur/focus can get coalesced by the browser with no gap between them
    });
  });
  function autoGrow(){
  ...
  input.addEventListener('input', function(){
    ...
    // 'focus' event handler â€” but clicking into an ALREADY-focused element
    ...
    // fires on every real keystroke regardless of focus history â€” the
  ...
  // WORKER_URL is now resolved once, near DEPLOYMENT_COMPANY_NAME above â€”
  ...
  // Must match index-worker.js's own PIN_PROMPT_TEXT constant exactly â€”
  ...
  const PIN_PROMPT_TEXT = "Sure â€” what's your PIN?";
  ...
  //
  // Persistent footer (Section 12) â€” appended ONCE as a direct sibling
  ...
  // keep it last â€” fragile by construction, since any code path that
  ...
  function ensureFooter(){
    ...
    // (1 September 2026 correction) â€” the footer belongs to the chat
    ...
    footer.querySelector('#askFooterCopy').addEventListener('click', function(){
      ...
      // request (1 September 2026 correction) â€” the "Requestingâ€¦"
  ...
  // progress, then sent" state machine â€” the existing backend contract
  ...
  function markFooterCopySent(){
    ...
    if (!btn) return;
    btn.textContent = 'âœ“ Conversation copy sent';
  ...
  // uppercase/arrow .ask-sysnote treatment either â€” a distinct third
  ...
  // the existing paths â€” this is specifically for the "clicked an
  // interface action" case, left-aligned, reading as "you clicked
  // [action]" rather than a spoken message. `label` is the bare action
  // name only (e.g. "Contact", "How to use LiveAsk") â€” every call site
  // was updated to pass this instead of the old pre-formatted
  // "â†’ You clicked X" string, so there is nothing left to parse or
  // strip here.
  ...
  // same message element â€” satisfies the required render order
  ...
  // if the content contains them â€” the CSS above supports rendering
  ...
  // pass â€” flagged here rather than silently added or silently skipped.
  ...
  // One stable element for the life of an active response â€” created
  // once per turn, its emoji/text content changed in place for the
  // ðŸ¤” -> ðŸ™‚ transition, never removed and recreated mid-turn. The
  ...
  function beginIdentity(){
    ...
    // uses (.ask-liveask-inline) â€” not the "LiveAsk AI" text label this
  ...
  // the element â€” three separate steps specifically so nothing ever
  ...
  // except their own action). BOTTOM_TOLERANCE is deliberately small â€”
  ...
  // function â€” auto-follow only applies while the visitor hasn't
  ...
  // FOLLOWING_LATEST â€” none of them call this function with any special
  ...
  function ensureScrollLatestButton(){
    ...
    // attempt) has now failed in actual observed rendering too â€” not
    ...
    // NOT scroll (only #askThread inside it does) â€” anchored with a
    ...
    // shape from Hide Chat's bare chevron â€” the two controls do
  ...
  // navigation but not a closed tab or a fresh one â€” exactly the lifetime
  ...
  function loadSession(){
    try {
      ...
      return null; // corrupt/unavailable storage â€” fall back to a fresh session, never throw
    }
  }
  function saveSession(){
    try {
      ...
      // Storage unavailable or full (private-browsing modes, quota) â€” the
  ...
  // handleTourCreate in index-worker.js â€” the guest link is literally
  ...
  // treat it as a brand new tour visit and start completely fresh â€” a tour
  ...
  // instant the visitor actually opens the panel again â€” but does NOT
  ...
  // are now two separate concerns â€” content persists always; visibility
  ...
  // link â€” see each of those call sites for their own .expanded handling).
  ...
  // â€” they'd be wrong the moment there's already a real conversation on
  ...
  function replaySession(){
    ...
    // Persistent chat-copy control â€” restored on a same-tab reload/nav same
    ...
    // saveSession to begin with) â€” explicit clear here just guarantees
  ...
  // in sync with TOUR_DESTINATIONS in index-worker.js â€” the Worker only
  ...
  // `page` (the real site path it lives on) alongside `selector` â€” until 25
  ...
  // Persists across the hard navigation a cross-page GO_TO needs â€” a real
  ...
  // quickReplies (added 26 August 2026 â€” real bug found live: "The Tour
  ...
  // navigation fires â€” a real page load destroys that DOM before the
  ...
  // The actual same-page scroll-and-highlight â€” unchanged from the original
  ...
  function scrollAndHighlight(selector){
    ...
    // (position: sticky) at the top of the viewport at all times â€” a plain
  ...
  function tourMutedStatus(){
    return tourPlaybackState === 'COMPLETED'
      ? 'Tour concluded â€” ask me anything'
      : 'Voice ready â€” microphone muted';
  }
  ...
  function showTourMedia(asset, actionTarget){
    ...
    function finish(eventName){
      ...
      if (voiceSessionIsOpen()) setVoiceUi('muted', 'Preparing tour wrap-upâ€¦');
  ...
  // unknown destination name (never breaks the reply that came with it) â€”
  ...
  function handleTourAction(action, pendingQuickReplies){
    ...
    // same reply, so nothing more needs saying â€” just remember what's
    ...
    // (26 August 2026) rides along the same way â€” see savePendingTourAction's
  ...
  // via a tour link,
  // not organically â€” fetch and show the fixed greeting instead (see
  ...
  function beginTourEntry(){
    ...
    fetch(WORKER_URL, {
      ...
      .then(function(data){
        ...
        const replyText = data.reply || "Welcome! Something went wrong setting up your tour â€” try refreshing, or just ask a question below.";
        ...
        // did) â€” so the Worker's new fixed "Start tour" button silently had
        ...
        // landed â€” before they'd tapped or typed anything at all. On a
        ...
        // submitToPanel above â€” only a genuine typed submission should
      ...
      .catch(function(){
        ...
        a.className = 'ask-msg ai';
        a.innerHTML = '<p></p>';
        a.querySelector('p').textContent = "That's taking longer than it should to load your tour â€” try refreshing, or just ask a question below.";
        thread.appendChild(a);
        renderRow2([]);
        showFooter();
        maybeScrollToBottom();
  ...
  // out the scroll-and-highlight it was always meant to do â€” same
  ...
  // shifts height on load) settle before measuring positions â€” the
  ...
  //
  // Any mismatch â€” no pending action, unknown target, or a pending
  // action whose page doesn't match here (shouldn't happen, but a
  // guest could always intervene by hand) â€” is a silent no-op, same
  ...
  //
  // pendingQuickReplies (added 26 August 2026 â€” see savePendingTourAction's
  ...
  // built set â€” same renderRow2/submitToPanel machinery as every other
  // quick-reply row in the app â€” renders into Row 2 here instead, once the
  ...
  (function(){
    ...
    // deliberate action â€” per Chris, this should deterministically reveal
  ...
  // page it's clicked from â€” "#about" when already on the homepage,
  ...
  // "Close" button â€” 10 August 2026. Purely visual collapse: removes the
  ...
  // re-add .expanded/.active on every new message â€” so the next message
  ...
  // which resets the JS variables themselves â€” nothing in this button
  // does that.
  //
  ...
  // panel VISIBILITY were wrongly tied together â€” see replaySession's own
  ...
  // search-bar-like control on the web â€” focusing the input reveals what's
  ...
  // redesign) â€” replaced by the persistent footer's "Hide Chat âŒƒ"
  ...
  // menu, every Admin/Tour secondary panel â€” all appended as descendants
  // of askPanel's own .ask-box, so none of them should close it), and any
  // of the site's own defined nav-intent triggers (About, Contact, etc. â€”
  ...
  // click and fight each other. Every other click on the page â€” page text,
  // whitespace, a plain link that isn't a defined trigger â€” collapses it.
  //
  ...
  // outside that tree â€” e.target here no longer reports the actual
  ...
  // tree, in order â€” checking that path directly against the real
  ...
  // deterministic â€” never generated by the model (see
  ...
  // information (a model-emitted tag plus a regex fallback) â€” real testing
  ...
  // for something â€” showing it unconditionally on the session's very first
  ...
  // conversationHistory â€” must always be called BEFORE that turn is pushed,
  ...
  // re-shows after a page-hop â€” it already ran once, earlier in this same
  ...
  // Renders the model's offered closed-choice buttons (data.quickReplies â€”
  // see system-prompt.js "Guided (closed) questions â€” Quick Replies" and
  // Section 8). Distinct from NAV_INTENTS: a Quick Reply click is ordinary
  // conversational input, not a fixed opener â€” it goes back through the
  ...
  // Actions (lead capture, verification) â€” selecting a choice never itself
  ...
  // inside whichever AI message bubble had just landed â€” meaning the
  ...
  // CURRENT turn's valid choices â€” cleared and rebuilt on every call, never
  // tied to a specific historical bubble. The '+' placeholder to its left
  // is static markup, not touched here â€” it's a reserved-but-inert spot
  ...
  // working around â€” a self-removing button detaching itself from the
  ...
  function renderRow2(choices){
    ...
    (choices || []).forEach(function(choice){
      ...
      btn.addEventListener('click', function(){
        // Scoped to quickreply buttons only â€” mic/send now live in this
  ...
  // Defensive client-side re-validation of data.quickReplies â€” the Worker
  ...
  // Shared core â€” both manual typing and nav-triggered prompts flow through
  ...
  // or stays invisible behind a plain system note (nav clicks) â€” a nav click
  ...
  function submitToPanel(promptText, opts){
    ...
    // was the AI's most recent turn â€” deterministic, no guessing at intent,
    ...
    // server this one time (that's the whole point â€” the server is what
    ...
    // own length â€” a variable-length mask would leak how many digits it
    // was), and conversationHistory â€” which gets saved into this browser's
    // own sessionStorage AND resent in full on every later request â€” never
    ...
    if(opts.showVisitorBubble !== false){
      ...
      v.className = 'ask-msg visitor';
      v.innerHTML = '<p></p>';
      v.querySelector('p').textContent = isPinAnswer ? 'â€¢â€¢â€¢â€¢â€¢â€¢' : promptText;
      thread.appendChild(v);
    }
    ...
    // thinking-bubble append and scroll above â€” both real DOM mutations
    ...
    // be unconditional â€” meaning it ran even when this turn came from a
    ...
    // path (send(), via opts.refocusInput) â€” a quick-reply choice never
    ...
    // ever holds the redacted placeholder) â€” the one and only place the
    ...
    fetch(WORKER_URL, {
      ...
      .then(function(data){
        ...
        applyTourStatePresentation(data);
        const replyText = data.suppressReply ? '' : (data.reply || "Something went wrong on my end â€” try again in a moment.");
        // Must be checked BEFORE this reply is pushed to conversationHistory
        // below â€” see isFirstAiReply().
        const showPrivacyNotice = isFirstAiReply();
        if (replyText) {
          conversationHistory.push(conversationMessage('assistant', replyText, data.canonicalEvent || {
            turn_id: textTurnId,
        ...
        a.className = 'ask-msg ai';
        a.innerHTML = '<p></p>';
        // Capture the reply <p> BEFORE inserting the notice paragraph â€” real
        ...
        // is now first in DOM order (the notice), not the reply â€” silently
        ...
        // state machine in index-worker.js's fetch()) â€” undefined/absent
        ...
        // mutation that can reset the caret blink a second time â€” same
        ...
        // (25 August 2026) â€” same reasoning: don't summon the phone
      ...
      .catch(function(){
        ...
        a.className = 'ask-msg ai';
        a.innerHTML = '<p></p>';
        a.querySelector('p').textContent = "That's taking longer than it should â€” please try again in a moment.";
        thread.appendChild(a);
        renderRow2([]);
        showFooter();
        maybeScrollToBottom();
  ...
  function send(){
    ...
    updatePrimaryControlState();
    updatePrimaryControlState();
    // refocusInput: true â€” this is the one real "the visitor was just
  ...
  // exact same tested pipeline as if the visitor had typed it themselves â€”
  ...
  // homepage â€” genuinely tenant page content bundled into this
  ...
  // ---- Nav-intent triggers ("Services", "About", "Contact", etc.) â€” the
  ...
  // deployment. Genericized 1 September 2026 â€” this previously held
  ...
  // written so it cannot be mistaken for real business data â€” replace
  ...
  // HTML â€” that markup lives on their site, not in this file.
  const NAV_INTENTS = {
    services: {
      note: 'Services',
      reply: "Here's an overview of what we offer:\n\nExample Service A â€” from $X Â· one or two sentences describing what this service does and who it's for\nExample Service B â€” from $X Â· one or two sentences describing what this service does and who it's for\nExample Service C â€” from $X Â· one or two sentences describing what this service does and who it's for\n\nWhich would you like to know more about?"
    },
    about: {
      note: 'About',
      reply: "A short paragraph introducing this business â€” who they are, what they do differently, and why a visitor should care. Replace with the deployment's real About copy.",
    ...
    'book-audit': {
      ...
      reply: "Thank you for your interest â€” may I have your name and business name please?",
      inputInstruction: { kind: 'name-business', label: 'Enter your name and business name' }
    },
    'enquire-build': {
      note: 'Example Enquiry B',
      reply: "Thank you for enquiring â€” may I have your name and business name please?",
      inputInstruction: { kind: 'name-business', label: 'Enter your name and business name' }
    },
    'register-protocol': {
  ...
  // reply path â€” no per-entry flag needed any more.
  host.$$('[data-nav-intent]').forEach(function(el){
    el.addEventListener('click', function(e){
      ...
      // text from immediately re-fading) â€” but that also skips the focus
      ...
      // Contact â€” it kept firing every 8s in the background indefinitely,
      ...
      a.className = 'ask-msg ai';
      a.innerHTML = '<p></p>';
      // Capture the reply <p> BEFORE inserting the notice â€” see the matching
      ...
      // further down â€” see isFirstAiReply().
      const replyP = a.querySelector('p');
      if (isFirstAiReply()) {
        a.insertBefore(buildPrivacyNoticeEl(), replyP);
      }
      replyP.textContent = cfg.reply;
      // Optional real, genuinely clickable link â€” a separate element, not
      ...
      // the normal, already-tested flow â€” no separate Claude call for
      ...
      // â€” but clicking a nav button (About/Services/Contact etc.) is a
  ...
  // ====================================================================
  // One shared popover shell â€” a discreet upward-opening popover on
  // desktop, a capped-height bottom sheet with a scrim on mobile (spec
  // Section 4) â€” reused for the public `+` menu itself, the Secondary Input
  ...
  // established pattern as refreshChatCopyLink/buildPrivacyNoticeEl above â€”
  ...
  // into a rigid form wizard") â€” Admin's "Create Tour" below is only a
  ...
  // â€” Section 3.4 explicitly lists "small contextual choice sets" as a
  ...
  // Admin session state â€” mirrors the server's adminsession:<sessionId>
  ...
  // server's own 30-minute TTL â€” a closed panel means this sitting is over.
  ...
  // Renders one "screen" into the shared popover â€” a title, an optional
  ...
  // opts.progress = { step, total } â€” Phase 2 UI refinement pass: renders
  // a small "n/total" + filled bar above the title, used by the two-step
  // Give Feedback flow. Purely presentational â€” carries no state of its
  ...
  // Phase 2 UI refinement pass â€” full-width navigation rows with a trailing
  ...
  // else â€” never an ordinary row, per the pass brief's explicit carve-out.
  ...
  // opts.multiline â†’ <textarea> instead of <input> (Give Feedback's comment
  // step wants a generously sized field, Section 4 of the pass brief).
  // Autofocuses on render and, when opts.onEnter is given, Enter submits â€”
  ...
  // burgundy .btn--primary â€” confirmed 28 August 2026: burgundy stays
  ...
  // Generic POST-and-parse helper â€” every Admin/Feedback/Restart request
  ...
  // ---- Public Customer Quick Menu (Section 6) â€” fetched once per page
  ...
  // needing a reload. No admin auth on this request â€” every visitor sees
  // it. ----
  ...
  // Fixed, code-authored explanation â€” no Claude call, same "code decides
  ...
  function showHowToUseLiveAsk(){
    ...
    // service' (1 September 2026 correction) â€” that phrasing assumed a
  ...
  // August 2026 â€” see the v3 Addendum) â€” openPlusMenu below never even
  ...
  // pass, Sections 3-4) â€” quality/interaction reference only, LiveAsk's own
      ...
      renderActions(body, [
        { label: 'Submit', primary: true, onClick: function(){
          postWorker({ giveFeedback: { sessionId: sessionId, rating: rating, comment: field.value } })
            ...
            .catch(function(){ renderPopoverError(body, "That didn't send â€” please try again."); });
        } },
        { label: 'Cancel', onClick: closePlusMenu }
      ]);
  ...
  // ---- LiveAsk section item: Restart Tour (Section 5.1.D) â€” contextual
  ...
  function adminPinEntry(){
    renderSecondaryPanel('Admin', function(body){
      ...
      function submitPin(){
        ...
        // field never touching messages/conversationHistory â€” see
  ...
  function adminMenu(){
    renderSecondaryPanel('Admin â€” ' + adminAuthed.raName, function(body){
      renderNavRows(body, [
  ...
  // ---- Create Tour (Section 8.3) â€” hands off into the SAME proven
  ...
  // panel, not the Secondary Input Layer â€” exactly as if they'd typed
      ...
      adminAction('manageToursList').then(function(data){
        ...
        function renderGroup(label, list){
          ...
          list.forEach(function(t){
            ...
            row.innerHTML = '<div class="txt"><div class="ttl"></div><div class="meta"></div></div><span class="chev" aria-hidden="true">â€º</span>';
  ...
  // Tour Detail (Phase 2 UI refinement pass, Section 8) â€” a proper detail
  ...
  // exception in adminManageToursReview (index-worker.js) â€” Active/Expired
  ...
  // Run/Test â€” pages through the server's already-computed narration for
  ...
  // Edit â€” an ordered tap-to-pick multi-select over all 4 possible
      ...
      adminAction('manageToursEditOptions', { token: token }).then(function(data){
        ...
        function renderList(){
          ...
          data.allDestinations.forEach(function(d){
            ...
            btn.textContent = (order === -1 ? 'â˜ ' : ('â˜‘ ' + (order + 1) + '. ')) + d.picker;
            btn.addEventListener('click', function(){
  ...
  // Revoke â€” MVP behaviour, explicitly documented as such (see
  ...
  // values verbatim â€” a real risk of the two silently drifting apart
  ...
  // â€” that's an admin-UI clarity choice, not customer content, and isn't
  ...
  // One-line secondary summary shown under an item's title in the list â€”
  // Phase 2 UI refinement pass, Section 9 â€” built entirely from fields the
  ...
  // editing is explicitly deferred â€” this pass only restructures how an
  ...
  function adminQuickMenuItemDetail(item){
    renderSecondaryPanel(item.title, function(body){
      ...
      [['Type', typeLabel], ['Detail', valueLabel || 'â€”']].forEach(function(pair){
        const row = document.createElement('div');
        row.className = 'ask-popover-detail-row';
        const k = document.createElement('span');
        k.className = 'k';
  ...
  // Phase 2 UI refinement pass (Section 1): the customer/company section â€”
  // divider, tenant heading, and its items â€” is only ever appended
  ...
  // "Nothing here yet." message) â€” the menu just ends after LiveAsk's own
  // items, rather than flashing a "Loadingâ€¦" placeholder first.
  function openPlusMenu(){
    ...
    renderSecondaryPanel(null, function(body){
      ...
      // Start new chat â€” omitted entirely while a Tour is active (Section
      ...
      // Restart Tour â€” contextual only (Section 5.1.D): shown only while a
  ...
  // Reuse the panel's own existing outside-click-to-collapse exclusion â€”
  ...
  // ====================================================================
  // Dictation + Realtime Voice client â€” 10 September 2026.
  //
  ...
  async function finishVoice(options){
    ...
    if (options.showEnding !== false && voiceMode !== 'idle') setVoiceUi('ending', 'Endingâ€¦');
    stopLocalVoiceMedia();
  ...
  function handleVoiceControlMessage(event){
    ...
    if (data.type === 'sideband.attached') {
      if (voiceAttachTimer) { clearTimeout(voiceAttachTimer); voiceAttachTimer = null; }
      if (!voiceMuted) setVoiceUi(pendingTourVoiceCommand ? 'speaking' : 'listening', pendingTourVoiceCommand ? 'Starting Tourâ€¦' : 'Listeningâ€¦');
      else setVoiceUi('muted', pendingTourVoiceCommand ? 'Starting Tourâ€¦' : tourMutedStatus());
      return;
    }
    if (data.type === 'voice.state.speaking') {
      if (!voiceMuted && voiceMode !== 'speaking') setVoiceUi('listening', 'Listeningâ€¦');
      return;
    }
    if (data.type === 'voice.state.responding') {
      if (data.responding) {
        setVoiceUi('speaking', 'Speakingâ€¦');
        if (!voiceIdentityEl) {
          voiceIdentityEl = beginIdentity();
          setTimeout(function(){ if (voiceIdentityEl) beginAnswering(voiceIdentityEl); }, 350);
        }
      } else if (!voiceMuted) {
        setVoiceUi('listening', 'Listeningâ€¦');
      } else {
    ...
    if (data.type === 'voice.turn.incomplete' || data.type === 'voice.turn.ended') {
      ...
      if (!voiceMuted) setVoiceUi('listening', 'Listeningâ€¦');
  ...
  async function startVoice(options){
    ...
    clearInterval(rotateTimer); rotateTimer = null;
    clearTimeout(rotateFadeTimeout);
    setVoiceUi('connecting', 'Connectingâ€¦');
    ...
    try {
      ...
      channel.addEventListener('message', function(event){
        ...
        if (providerEvent.type === 'output_audio_buffer.started') setVoiceUi('speaking', 'Speakingâ€¦');
  ...
  function toggleVoiceMute(){
    ...
    setVoiceUi(voiceMuted ? 'muted' : 'listening', voiceMuted ? 'Microphone muted' : 'Listeningâ€¦');
  }
